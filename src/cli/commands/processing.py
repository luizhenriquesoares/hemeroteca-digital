...
def register_processing_commands(cli) -> None:
    ...
    @click.option("--bib", default=None, help="Código do acervo específico")
    @click.option("--provider", default="openai", type=click.Choice(["openai", "claude"]))
    ...
    def corrigir(bib, provider, model, force, workers):
        """Corrige textos OCR usando OpenAI ou Claude."""
    ...
    def corrigir_claude_legacy(bib, model, force, workers):
        ...
        processados, falhas = run_parallel_file_jobs(
            arquivos,
            lambda arquivo: corrigir_arquivo(arquivo, "claude", model, force),
            workers,
        )
        console.print(f"\n[bold green]Concluído: {processados} corrigidos, {falhas} falhas[/bold green]")
    ...
    def chunkar(bib, force):
        ...
        from src.processing.chunker import criar_chunks_acervo, criar_chunks_todos

        console.print("\n[bold]Criando chunks...[/bold]\n")
        if bib:
            console.print(f"Acervo {bib}: {criar_chunks_acervo(bib, force=force)} chunks criados")
            return
    ...
    def indexar(bib):
        ...
        from src.processing.indexer import indexar_acervo, indexar_todos

        console.print("\n[bold]Indexando no ChromaDB...[/bold]\n")
        if bib:
            console.print(f"Acervo {bib}: {indexar_acervo(bib)} chunks indexados")
            return
